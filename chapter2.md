...
## curl コマンド
第一引数に指定されたURLにアクセスして、コンテンツを取得するコマンド
...
## THHP
HTMLなどの情報をやりとりするTCP/IP上のプロトコル
- HTTPSはTHHPを暗号化したもの
- DNSはホスト名をIPアドレスに引き換えてくれる

## Bot
- cronプログラムを決められたスケジュールに合わせて自動実行してくれるプロセス

コマンド: crontab -e

一番下の数字が毎分何分に実行するか決める.  
Escを押して :wqと入力してEnter

# GItHub

## イシュー管理
GitHubではイシューとして取りまとめて、数多くの人で議論をする機能がある

## Wiki
Weeb上から簡単に内容を書き換えられるWeb管理サイトのこと

- Gistという、Wikiほの本格的なページが要らない場合に利用できるサービスもある

## コマンド
- git add: コマンドで、リポジトリに変更情報を追加する→このことをステージングと言う
- git commit -m: コマンドで、リポジトリのインデックスに追加された変更情報にコメントを付けてコミットできる
- git push: コマンドで、ローカルコミットをリモートリポジトリに反映させることができる
- git pull origin ^^^: リモートリポジトリの変更をローカルリポジトリに取り込むこと

## branch(ブランチ)
ある時点のソースコードを分岐させて開発したもの

- git brabch: このリポジトリにどのようなブランチがあるかを、一覧表示する
    - "*" マークがついているブランチが現在利用しているブランチを表している
- git brabch ^^^^: ブランチの作成コマンド。
    - ^^^^のところに付けたいブランチ名。
- git checkout ^^^^: ブランチの切替
     - ^^^^のところに付けたいブランチ名。

## merge(マージ)
他のブランチの変更情報を別のブランチに取り込むこと
1. マージを取り込みたいブランチに移動
1. git merge ^^^ (^^^は取り込みたいブランチ名)

同じ箇所を別の人が変更するとマージできない。これをコンフリクトという
